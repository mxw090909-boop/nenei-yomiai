...
await click('页边');
await click('回到原文');
assert.ok(w.document.querySelector('[data-paragraph-index="5"] .reader-jump-mark'), 'note jump highlights its paragraph when quotation is no longer exact');
await delay(3200);
assert.equal(w.document.querySelector('.reader-jump-mark'), null, 'arrival highlight expires');
await click('回到首页');await click('设置');
const homeVeil=w.document.querySelector('[aria-label="页面遮罩浓度"]');
assert.equal(homeVeil.value,'24','existing home opacity retained by default');
assert.equal(w.document.querySelector('[aria-label="阅读遮罩浓度"]').value,'30');
Object.getOwnPropertyDescriptor(w.HTMLInputElement.prototype,'value').set.call(homeVeil,'52');
homeVeil.dispatchEvent(new w.Event('input',{bubbles:true}));homeVeil.dispatchEvent(new w.Event('change',{bubbles:true}));await delay(30);
assert.equal(w.localStorage.getItem('nenei-yomiai-home-bg-opacity'),'52','slider persists opacity');
assert.equal(w.document.documentElement.style.getPropertyValue('--yomiai-home-veil'),'0.52','slider updates background immediately');
await click('首页');await click('设置');
assert.equal(w.document.querySelector('[aria-label="页面遮罩浓度"]').value,'52','opacity survives page navigation');
